...
Run multiple tests by using logical `&&` and `||` operators:
```sh
...
```sh
# Smoke only
npx playwright test tests/ --grep "@smoke"

# API tests only
npx playwright test tests/ --grep "@api"

# PAX-related tests
npx playwright test tests/ --grep "@pax||@PAX"

# Regression, excluding setup and api
npx playwright test tests/ --grep-invert "@setup&&@api"
```
# Local Report
