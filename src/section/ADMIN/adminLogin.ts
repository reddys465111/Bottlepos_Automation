...
export class Admin_Login {
    ...
    public async FormNotVisible(): Promise<boolean> {
        // const loginmodal = this._page.locator('#loginmodal:not([style$="display: none;"])').waitFor({timeout: 3000});
        // return await this._page.locator('#loginmodal:not([style$="display: none;"])').count() > 0;
        return await this._page.locator('#loginmodal').isHidden({timeout: 3000});
    }
