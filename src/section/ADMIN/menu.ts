...
export class MenuItem extends BaseObject{
    ...
    public async GoTo(): Promise<void>{
        if(this._parent){
            // console.log('Menubar is collapsed? : '+ await this.MenuIsCollapsed());
            if(await this.MenuIsCollapsed()){
                await this._parent.Hover();
            }else{
                // console.log('get parent attribute');
                
                const submenuOpen = await this._parent?._locator.getAttribute('class');
                // console.log('attribute = '+ submenuOpen);
        ...
        // try{
        //     await this._page.locator('.dataTables_processing[style*="display: block"]').waitFor({timeout: 5000});
        //     await this._page.locator('.dataTables_processing[style*="display: none"]').waitFor({timeout: 5000});
        // }catch(e){}
    }
