import { Locator, Page } from "@playwright/test";
import { Table_Pagination } from "../../frontEnd/tables";
import { Dialog_ManageCategoryGroup } from "../../dialogs/dialog_ManageCategoryGroup";
import { Dialog_AddCategory } from "../../dialogs/dailog_AddCategory";
import { Table_Categories } from "../../tables/table_Categories";
import { Button } from "../../../../objects/button";
import { Dropdown } from "../../../../objects/dropdown";
import { TextField } from "../../../../objects/textField";


export class Admin_Categories{

    public Home: Button;
    public Referesh: Button;
    public ManageCategoryGroup: Button;
    public Add: Button;
    public Search: TextField;
    public ShowEntries: Dropdown;
    public Table: Table_Categories;
    public ShowingEntries: Table_Pagination;
    public Dialog_ManageCategoryGroup: Dialog_ManageCategoryGroup;
    public Dialog_AddCategory: Dialog_AddCategory;

    constructor(page: Page){

        this.Home = new Button(page.locator("li a[onclick='WPOS.goToHome();']"));
        this.Referesh = new Button(page.locator('[title="Refresh"]'))
        this.ManageCategoryGroup = new Button(page.locator('[title="Manage Category Group"]'));
        this.Add = new Button(page.locator('[title="Add"]'));
        this.Search = new TextField(page.locator("input.form-control.input-sm[aria-controls='categoriestable']"));
        this.ShowEntries = new Dropdown(page.locator("select[name='categoriestable_length']"));
        this.Table = new Table_Categories(page.locator('#categoriestable'));
        this.ShowingEntries = new Table_Pagination(page.locator('#categoriestable_paginate'));
        this.Dialog_ManageCategoryGroup = new Dialog_ManageCategoryGroup(page);
        this.Dialog_AddCategory = new Dialog_AddCategory(page);
        
    }
}