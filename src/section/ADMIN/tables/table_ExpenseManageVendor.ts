...
export class table_ManageVendor extends BaseTable<titles> {
   ...
   constructor(locator: Locator, page?: Page) {
    ...
    public async Delete(...rowQuery: RowQuery<titles>[]): Promise<void> {
           ...
           if (rowQuery && rowQuery.length > 0) {
               ...
               if (this._page) {
                   try {
                       const searchInput = this._page.locator('#managevendortable_filter input[type="search"]');
       ...
       public async IsRecordDeleted(timeout = 10000): Promise<boolean> {
           ...
           // Wait for either rows to load or the empty message to appear
