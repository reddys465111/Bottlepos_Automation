...
export class BottlePOS {
    ...
    public MobileHamburgerMenu!: Button;
    ...
    public async Initialize(page: Page, device: string):Promise<void> {
        ...
        this.MobileHamburgerMenu = new Button(this._page.locator('[aria-label="Toggle menu"]'));
       //Handle Admin button locator based on the device type
        if (device === 'mobile') {
            this.Admin = new Button(this._page.getByRole('link', { name: 'Admin' }));
        } else {
        ...
        let customerScreenPagePromise: Promise<Page>;
        ...
        this.CustomerScreenButton.SetBeforeEvent(async ()=> {
            customerScreenPagePromise = this._page.context().waitForEvent('page');
        })
        this.CustomerScreenButton.SetAfterEvent(async ()=>{ 
            await this.ChangeToCustomerScreen(customerScreenPagePromise);
        });
        ...
        // Handle Logout button locator based on the device type
        if (device === 'mobile') {
            this.Logout = new Button(this._page.locator('button:has(.fa-power-off)').first());
        } else {
    ...
    public async ChangeToCustomerScreen(customerScreenPagePromise: Promise<Page>): Promise<void>{
        const customerScreenPage = await customerScreenPagePromise;
        await customerScreenPage.bringToFront();
        // Update CustomerScreen instance with the new page
        this.CustomerScreen = new CustomerScreen(customerScreenPage);
        // Wait for the first item to be loaded on the customer screen
        await this.CustomerScreen.WaitForFirstItem();
    }
    /** 
