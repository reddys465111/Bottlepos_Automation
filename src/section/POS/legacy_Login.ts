...
export class Legacy_Login {
     ...
     */
    public async FormVisible(timeout = 10000): Promise<boolean> {
        try {
    ...
    public async WaitForLoadingBar(): Promise<void> {
        try {
        await this._page.getByText('Getting device settings...').waitFor({ state: "attached" , timeout: 20000});
        } catch (error) {return;}
        try {
            await this._page.getByText('Getting device settings...').waitFor({ state: "hidden" , timeout: 20000});
        } catch (error) {return;}
        try {
            await this._page.getByText('Getting Stored Items...').waitFor({ state: "attached" , timeout: 20000});
        } catch (error) {return;}
        try {
            await this._page.getByText('Getting Stored Items...').waitFor({ state: "hidden" , timeout: 20000});
        } catch (error) {return;}
        try {
            await this._page.getByText('Getting Customer Accounts...').waitFor({ state: "attached" , timeout: 20000});
        } catch (error) {return;}
    }
    ...
    public async In(options?: { user?: string, password?: string, device?: string, location?: string, reloadURL?: boolean, validateForm?: boolean }): Promise<void> {
        ...
        if (await POS.Dialog.Legacy_InitialDeviceSetup.IsVisible() && (options?.reloadURL !== undefined || options?.reloadURL !== false)) {
            ...
            await POS.Dialog.Legacy_InitialDeviceSetup.IsNotVisible();
