...
import { Device_Get, DeviceAndLocation_Get } from "../API/useCases/ADMIN/settings/deviceAndLocations/devices";
...
import { PAXMock } from "../paymentDevices/pax/paxMock";
import { injectGiftCardItemIntoItemsMock } from "../API/useCases/giftCards/giftCard";
import { injectNativeGiftCardEnabled } from "./configMock";
...
export interface initArgs {
    ...
    Scenario?: iScenario,
    PaymentDevice?: { name: 'PAX' | 'Payfac' | 'none'}
}
...
class InitializerClass {
    ...
    public async Init(page: Page, args?: initArgs): Promise<void> {
        if (args?.PaymentDevice?.name === 'PAX') {
            await PAXMock.injectMock(page, {
                autoApprove: true
              });
        }
        
        if(args?.Scenario?.Admin?.Settings?.GeneralSettings?.GiftCards) {
            await injectNativeGiftCardEnabled(page);
        ...
        if(args?.PaymentDevice?.name === 'Payfac') {
            await payfac.Init(page);
        }
        if(args?.PaymentDevice?.name === 'PAX') {
            await PAXMock.Initialize(page);
        }
    }
    ...
    private async LoadURL(page: Page, args?: initArgs): Promise<void> {    
        ...
        if (!args?.Admin) {
            ...
            await page.addInitScript(() => {
                localStorage.setItem('WPOS_CurrentUIVersionIdentifier', '2.0');
            });
        }
        ...
        // Wait for application to be ready. Use 'load' as fallback when offline so we don't hang on networkidle.
        try {
    ...
    public async LoadScenario(args?: iScenario): Promise<void> {
        if (args) {
            ...
            const deviceId = await Device_Get(Session.Device.Name, {
                locationId: locationId?.id != null ? String(locationId.id) : undefined,
                locationName: Session.Location.Name,
            });

            // Search for location with name "Inventory"
            // const locations = DeviceAndLocation?.["locations/get"];
            // const inventoryLocation = locations ? Object.values(locations).find((l: any) => l.name === Session.Location.Name.split('(')[0].trim()) : undefined;
            Session.Location.Id = locationId?.id ? String(locationId.id) : '';
            // // Search for device with the specified name
            // const devices = DeviceAndLocation?.["devices/get"];
            // const device = devices ? Object.values(devices).find((d: any) => d.name === Session.Device.Name.split('(')[0].trim()) : undefined;
            Session.Device.Id = deviceId?.id ? String(deviceId.id) : '';
