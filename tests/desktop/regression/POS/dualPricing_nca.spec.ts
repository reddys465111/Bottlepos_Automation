...
test.describe("TestRail POS Test Cases Dual Pricing and nca", { tag: ['@DualPricing', '@nca','@regression','@nonparallelizable'] }, () => {
    ...
    test('[C4036] Verify split payment with Payfac and Dual Pricing', { tag: ['@DualPricing', '@PayFac', '@Splitpay'] }, async ({ page }) => {
         await Initializer.Init(page, {
        Scenario: {
            Admin: {
                Settings: {
    ...
    await payfac.Init(page);
        // Login and open register
        await POS.Login.In();
        await POS.Register.Click();
        //  Ring up an item and verify it's on the register
        await POS.Register.AddItemByStockcode({ stockCode: ITEMS.JACK.BARCODE });
        // Click NCA Pay
        await POS.Register.PayButton.Click();
        // Get total and calculate a partial tender amount (half)
        const totalStr = await POS.Dialog.Checkout.CashTotal.getText();
        const total = Reports.parseCurrency(totalStr);
        //Do PArtail amount
        const partial = Math.round((total / 2) * 100) / 100;
        //  Enter partial cash tender and click Cash
        await POS.Dialog.Checkout.Tendered.setText({ value: partial.toString() });
        await POS.Dialog.Checkout.Cash.Click();
        // Verify Cash balance and remaining balance updated
        const regularBalance = Reports.parseCurrency(await POS.Dialog.Checkout.RegularBalance.getText());
        //  Click Card to pay remaining amount (simulate payfac)
        await payfac.Read({ amount: regularBalance });
        await POS.Dialog.Checkout.Card.Click();
        //  Complete the sale
        await POS.Dialog.Success.Close.Click();
        //Navigate to Sales
        await POS.Sales.Click();
        //Open latest transaction details
        await POS.Sales.transactions.Table.ViewTransactionDetail({ byIndex: 1 });
        //Verify payment details show both cash and card payments with correct amounts
        const firstPaymentMethod = await POS.Dialog.TransactionDetails.payments.method.getText();
        expect(firstPaymentMethod.toLowerCase(),'Partial Cash payment failed').toBe('cash');
        const secondPaymentMethod = await POS.Dialog.TransactionDetails.GetPaymentRow(1);
        // Validate second payment  NCA balance matches the non-cash balance from checkout and payment method is card
        const secondPaymentamount = Reports.parseCurrency(secondPaymentMethod.amount ?? '');
        expect(secondPaymentamount,'Dual price  balance does not match').toEqual(regularBalance);
        expect((secondPaymentMethod.method),'payfac Card payment method failed ').toContain('card');
