...
test.describe("TestRail POS Test Cases Dual Pricing and nca", { tag: ['@DualPricing', '@nca','@regression','@nonparallelizable'] }, () => {
    ...
    test('[C8269]  Verify split payment with Payfac and NCA', { tag: ['@NCA', '@PayFac', '@Splitpay'] }, async ({ page }) => {
        await Initializer.Init(page, {
            Scenario: {
                Admin: {
                    Settings: {
        ...
        const nonCashBalance = Reports.parseCurrency(await POS.Dialog.Checkout.NCABalance.getText());
        //  Click Card to pay remaining amount (simulate payfac)
        await payfac.Read({ amount: nonCashBalance });
        await POS.Dialog.Checkout.Card.Click();
        //  Complete the sale
        await POS.Dialog.Success.Close.Click();
        //Navigate to Sales
        ...
        expect(secondPaymentamount, 'NCA balance does not match').toEqual(nonCashBalance);
