...
test.describe("TestRail POS Test Cases", { tag: ['@uncategorized', '@regression'] }, () => {
    ...
    test('[C4397] Verify barcode scanner working for existing item', { tag: ['@barcode', '@valid'] }, async ({}) => {
    ...
    test('[C4035] Verify manual item is added to the sale when Add Item button is clicked', { tag: ['@manual'] }, async ({}) => {
        //Login to the POS application
        await POS.Login.In();
        //Click on the Add Item button (Manual Item)
        await POS.Register.AddItem.Click();
   ...
   test('[C1423] Verify if the item sale can be suspended from the POS side', { tag: ['@suspend', '@sale'] }, async ({}) => {
    ...
    test('[C1594] Verify if the access the admin side from the POS side', { tag: ['@admin', '@access'] }, async ({ context }) => {
    ...
    test('[C1595] Verify if the user can logout from the admin side', { tag: ['@logout'] }, async ({ context }) => {
        // Login the POS application
        await POS.Login.In();
    ...
    test('[C4032] Verify items with "Do not Discount" enabled are excluded from discount calculations', { tag: ['@discount'] }, async ({}) => {
        //Login to the POS application
        await POS.Login.In();
        //Enter the barcode of an item and hit enter, item will be ringed up
    ...
    test('[C2054] To verify the Login Page components', { tag: ['@login'] }, async ({}) => {
