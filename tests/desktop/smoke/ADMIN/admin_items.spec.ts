...
test.describe('Admin Section - Items', { tag: ['@smoke', '@admin', '@items'] }, () => {
    test.skip('[C1417] - Verify if the item can be added from the admin side', { tag: ['@new', '@review'] }, async ({}) => {
        // Login
    ...
    test('[C1421] - Verify if a list of items can be deleted from the admin side', { tag: ['@bulk-delete', '@noID'] }, async ({page}) => {
        // Login
        await ADMIN.Login.In();
