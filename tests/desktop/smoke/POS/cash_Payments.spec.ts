...
test.describe('POS Section - Cash Payments', { tag: ['@smoke', '@pos', '@cash', '@payments'] }, () => {
    ...
    test('[C1445] - Verify if the discount can be applied on the sale', { tag: ['@discount'] }, async ({}) => {
        ...
        expect(await POS.Register.ItemLines.GetDiscountTotal({ row: 1 }), 'Error: Discount is not applied').toEqual('2.00');
        expect(await POS.Register.ItemLines.IsDiscountHighlighted({ row: 1 }), 'Error: Discount is not highlighted').toBeTruthy();

        // Step 3: Remove discount
        await POS.Register.Discount.Remove({ row: 1 });
        expect(await POS.Register.ItemLines.GetDiscountTotal({ row: 1 }), 'Error: Discount is not removed').toBeFalsy();
        expect(await POS.Register.ItemLines.IsDiscountHighlighted({ row: 1 }), 'Error: Discount is still highlighted').toBeFalsy();
        ...
        expect(await POS.Register.ItemLines.GetDiscountText({ row: 1 }), 'Error: Discount is not applied').toContain('%');
        expect(await POS.Register.ItemLines.IsDiscountHighlighted({ row: 1 }), 'Error: Discount is not highlighted').toBeTruthy();
    });
});
