...
test.describe("Scenarios related to Refund", { tag: ['@mobile', '@cash', '@refund', '@smoke'] }, () => {
  
  test('[C204-1] Verify Refund Process with Cash Payment ', { tag: ['@register'] }, async ({}) => {
  ...
  test('[C294-1] Verify Refund process from sales ', { tag: ['@sales','@nonparallelizable'] }, async ({}) => {
    //Login to the POS application
    await POS.Login.In();
    //Enter the barcode of an item and hit enter, item will be ringed up
    ...
    expect(await POS.Sales.transactions.Table.GetCellValue({getValueFrom: "Status"}, {rowIndex: 1} ),'Transaction was not Refunded').toEqual('Refunded');
  });
